# Fill aether-usage-runtime backend spec

## Goal

Analyze the `aether-usage-runtime` crate and fill all spec files in `.trellis/spec/aether-usage-runtime/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-usage-runtime/`
- **Layer**: Services
- **Purpose**: Usage tracking: token counting, cost calculation, queue-based recording
- **Internal dependencies**: aether-ai-formats, aether-contracts, aether-data, aether-data-contracts, aether-runtime-state
- **Key patterns**: Event queue, async recording, cost formulas
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-usage-runtime"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-usage-runtime", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-usage-runtime", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-usage-runtime/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-usage-runtime/backend/`

# Fill aether-video-tasks-core backend spec

## Goal

Analyze the `aether-video-tasks-core` crate and fill all spec files in `.trellis/spec/aether-video-tasks-core/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-video-tasks-core/`
- **Layer**: Domain
- **Purpose**: Video generation task lifecycle: creation, polling, status tracking
- **Internal dependencies**: aether-contracts, aether-data-contracts
- **Key patterns**: Task state machine, async polling patterns
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-video-tasks-core"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-video-tasks-core", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-video-tasks-core", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-video-tasks-core/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-video-tasks-core/backend/`

# Fill aether-contracts backend spec

## Goal

Analyze the `aether-contracts` crate and fill all spec files in `.trellis/spec/aether-contracts/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-contracts/`
- **Layer**: Foundation
- **Purpose**: Shared trait contracts and interfaces across the system
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Trait definitions, no implementations
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-contracts"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-contracts", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-contracts", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-contracts/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-contracts/backend/`

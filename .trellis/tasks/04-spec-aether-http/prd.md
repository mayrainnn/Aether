# Fill aether-http backend spec

## Goal

Analyze the `aether-http` crate and fill all spec files in `.trellis/spec/aether-http/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-http/`
- **Layer**: Foundation
- **Purpose**: HTTP utilities and shared request/response types
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Thin wrapper types for axum
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-http"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-http", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-http", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-http/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-http/backend/`

# Fill aether-ai-formats backend spec

## Goal

Analyze the `aether-ai-formats` crate and fill all spec files in `.trellis/spec/aether-ai-formats/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-ai-formats/`
- **Layer**: Foundation
- **Purpose**: AI API format types: OpenAI, Claude, Gemini request/response structures
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Serde structs for multi-provider AI API formats, conversion traits
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-ai-formats"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-ai-formats", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-ai-formats", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-ai-formats/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-ai-formats/backend/`

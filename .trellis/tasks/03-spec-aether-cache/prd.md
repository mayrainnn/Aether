# Fill aether-cache backend spec

## Goal

Analyze the `aether-cache` crate and fill all spec files in `.trellis/spec/aether-cache/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-cache/`
- **Layer**: Foundation
- **Purpose**: TTL-based in-memory cache with namespace support
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Generic ExpiringMap<K,V>, CacheKeyNamespace enum
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-cache"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-cache", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-cache", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-cache/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-cache/backend/`

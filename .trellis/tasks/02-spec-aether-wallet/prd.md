# Fill aether-wallet backend spec

## Goal

Analyze the `aether-wallet` crate and fill all spec files in `.trellis/spec/aether-wallet/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-wallet/`
- **Layer**: Foundation
- **Purpose**: Wallet/balance abstractions for billing system
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Value types, balance operations
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-wallet"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-wallet", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-wallet", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-wallet/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-wallet/backend/`

# Fill aether-oauth backend spec

## Goal

Analyze the `aether-oauth` crate and fill all spec files in `.trellis/spec/aether-oauth/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-oauth/`
- **Layer**: Core
- **Purpose**: OAuth2 client implementations for provider authentication
- **Internal dependencies**: aether-contracts
- **Key patterns**: OAuth flow types, token refresh logic
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-oauth"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-oauth", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-oauth", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-oauth/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-oauth/backend/`

# Fill aether-ai-serving backend spec

## Goal

Analyze the `aether-ai-serving` crate and fill all spec files in `.trellis/spec/aether-ai-serving/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-ai-serving/`
- **Layer**: Services
- **Purpose**: AI request serving logic: format adaptation, model mapping, response normalization
- **Internal dependencies**: aether-ai-formats, aether-contracts, aether-scheduler-core
- **Key patterns**: Request/response adaptation pipeline, model alias resolution
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-ai-serving"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-ai-serving", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-ai-serving", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-ai-serving/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-ai-serving/backend/`

# Fill aether-runtime backend spec

## Goal

Analyze the `aether-runtime` crate and fill all spec files in `.trellis/spec/aether-runtime/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-runtime/`
- **Layer**: Core
- **Purpose**: Async runtime utilities: background task spawning, graceful shutdown
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: Tokio-based task management, JoinHandle tracking
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-runtime"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-runtime", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-runtime", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-runtime/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-runtime/backend/`

# Fill aether-admin backend spec

## Goal

Analyze the `aether-admin` crate and fill all spec files in `.trellis/spec/aether-admin/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-admin/`
- **Layer**: Application
- **Purpose**: Admin API handlers: provider management, system config, monitoring
- **Internal dependencies**: aether-ai-formats, aether-billing, aether-contracts, aether-data, aether-data-contracts
- **Key patterns**: Axum handlers, admin-only middleware, CRUD operations
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-admin"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-admin", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-admin", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-admin/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-admin/backend/`

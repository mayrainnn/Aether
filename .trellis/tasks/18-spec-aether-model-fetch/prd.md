# Fill aether-model-fetch backend spec

## Goal

Analyze the `aether-model-fetch` crate and fill all spec files in `.trellis/spec/aether-model-fetch/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-model-fetch/`
- **Layer**: Services
- **Purpose**: Model metadata fetching and caching from providers
- **Internal dependencies**: aether-ai-formats, aether-contracts, aether-data-contracts, aether-provider-transport, aether-scheduler-core
- **Key patterns**: Periodic refresh, model list aggregation
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-model-fetch"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-model-fetch", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-model-fetch", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-model-fetch/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-model-fetch/backend/`

# Fill aether-runtime-state backend spec

## Goal

Analyze the `aether-runtime-state` crate and fill all spec files in `.trellis/spec/aether-runtime-state/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-runtime-state/`
- **Layer**: Domain
- **Purpose**: Runtime state management: provider health, circuit breakers, rate limits
- **Internal dependencies**: aether-cache, aether-data-contracts, aether-runtime
- **Key patterns**: Shared mutable state behind Arc, health tracking
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-runtime-state"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-runtime-state", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-runtime-state", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-runtime-state/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-runtime-state/backend/`

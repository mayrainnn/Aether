# Fill aether-data-schema backend spec

## Goal

Analyze the `aether-data-schema` crate and fill all spec files in `.trellis/spec/aether-data-schema/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-data-schema/`
- **Layer**: Foundation
- **Purpose**: Database schema definitions (SeaORM entities)
- **Internal dependencies**: (none — leaf crate)
- **Key patterns**: SeaORM Entity/Model/ActiveModel, migration definitions
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-data-schema"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-data-schema", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-data-schema", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-data-schema/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-data-schema/backend/`

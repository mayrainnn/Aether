...
* Added a generic `browser_curl_cffi` transport backend to the execution-plan transport profile contract. Grok session-auth keys now default to this backend so quota refresh, chat/responses/messages, and image transit calls can use browser impersonation instead of plain `reqwest`.
* Added `scripts/browser_transport.py`, a generic sidecar that receives upstream method/URL/headers/body plus browser profile/proxy controls, then executes the request with `curl_cffi` browser impersonation. `dev.sh` autostarts it by default when `uv` is available.
...
* Reimplementing `curl_cffi` TLS/HTTP2 impersonation natively in Rust. The sidecar boundary is intentionally generic so it can be reused by future browser-fingerprint-sensitive providers.
...
* `cargo test -p aether-gateway direct_sync_execution_runtime_routes_browser_transport_via_sidecar --lib`
...
* `python3 -m py_compile scripts/browser_transport.py`
* Manual sidecar smoke test: `/health` returned `ok`; `/request` proxied `https://example.com/` with `chrome136` and returned HTTP 200.
...
* `cf_clearance` remains browser-profile, User-Agent, and proxy-affinity sensitive. Importing the same Cookie that works in `grok2api` now preserves the required profile material, but successful Grok calls still depend on using a compatible `curl_cffi` browser profile and the same usable network egress.

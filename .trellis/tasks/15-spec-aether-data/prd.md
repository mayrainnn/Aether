# Fill aether-data backend spec

## Goal

Analyze the `aether-data` crate and fill all spec files in `.trellis/spec/aether-data/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-data/`
- **Layer**: Domain
- **Purpose**: Data access layer: SeaORM repositories, Redis integration, query builders
- **Internal dependencies**: aether-ai-formats, aether-data-contracts, aether-cache, aether-wallet
- **Key patterns**: Repository implementations, connection pooling, transaction patterns
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-data"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-data", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-data", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-data/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-data/backend/`

# Fill aether-testkit backend spec

## Goal

Analyze the `aether-testkit` crate and fill all spec files in `.trellis/spec/aether-testkit/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-testkit/`
- **Layer**: Application
- **Purpose**: Integration test utilities: mock providers, test fixtures, assertion helpers
- **Internal dependencies**: aether-data, aether-contracts, aether-gateway, aether-http, aether-runtime, aether-runtime-state
- **Key patterns**: Test harness, mock server builders, fixture factories
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-testkit"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-testkit", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-testkit", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-testkit/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-testkit/backend/`

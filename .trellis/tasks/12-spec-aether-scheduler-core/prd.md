# Fill aether-scheduler-core backend spec

## Goal

Analyze the `aether-scheduler-core` crate and fill all spec files in `.trellis/spec/aether-scheduler-core/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-scheduler-core/`
- **Layer**: Domain
- **Purpose**: Provider scheduling: health checks, quota management, cooldown logic
- **Internal dependencies**: aether-ai-formats, aether-contracts, aether-data-contracts, aether-wallet
- **Key patterns**: Health/quota state machines, scheduling algorithms
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-scheduler-core"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-scheduler-core", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-scheduler-core", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-scheduler-core/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-scheduler-core/backend/`

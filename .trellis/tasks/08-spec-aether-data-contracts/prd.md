# Fill aether-data-contracts backend spec

## Goal

Analyze the `aether-data-contracts` crate and fill all spec files in `.trellis/spec/aether-data-contracts/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-data-contracts/`
- **Layer**: Core
- **Purpose**: Data layer contracts: repository traits, query types, domain models
- **Internal dependencies**: aether-ai-formats
- **Key patterns**: Repository trait pattern, domain model structs
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-data-contracts"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-data-contracts", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-data-contracts", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-data-contracts/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-data-contracts/backend/`

# Fill aether-billing backend spec

## Goal

Analyze the `aether-billing` crate and fill all spec files in `.trellis/spec/aether-billing/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-billing/`
- **Layer**: Services
- **Purpose**: Billing logic: plan enforcement, quota checks, payment integration
- **Internal dependencies**: aether-data-contracts, aether-usage-runtime
- **Key patterns**: Plan/quota types, billing event processing
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-billing"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-billing", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-billing", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-billing/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-billing/backend/`

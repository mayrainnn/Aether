# Fill aether-task-runtime backend spec

## Goal

Analyze the `aether-task-runtime` crate and fill all spec files in `.trellis/spec/aether-task-runtime/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-task-runtime/`
- **Layer**: Domain
- **Purpose**: Generic background task runtime with persistence
- **Internal dependencies**: aether-runtime
- **Key patterns**: Task trait, executor pattern
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-task-runtime"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-task-runtime", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-task-runtime", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-task-runtime/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-task-runtime/backend/`

# Fill aether-provider-transport backend spec

## Goal

Analyze the `aether-provider-transport` crate and fill all spec files in `.trellis/spec/aether-provider-transport/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `crates/aether-provider-transport/`
- **Layer**: Domain
- **Purpose**: HTTP transport to AI providers: request building, streaming, error mapping
- **Internal dependencies**: aether-ai-formats, aether-contracts, aether-crypto, aether-data-contracts, aether-oauth, aether-runtime-state, aether-video-tasks-core
- **Key patterns**: Provider adapters (OpenAI/Claude/Gemini/etc), streaming SSE, retry logic
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-provider-transport"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-provider-transport", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-provider-transport", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-provider-transport/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-provider-transport/backend/`

# Fill aether-proxy backend spec

## Goal

Analyze the `aether-proxy` crate and fill all spec files in `.trellis/spec/aether-proxy/backend/` with real, project-specific coding guidelines derived from the actual source code.

## Context

**Project**: Aether — a multi-provider AI API gateway written in Rust (axum + SeaORM + tokio).

**This crate**: `apps/aether-proxy/`
- **Layer**: Binary
- **Purpose**: Edge proxy binary: TLS termination, tunnel management, hardware detection, auto-registration
- **Internal dependencies**: aether-contracts, aether-http, aether-runtime, aether-runtime-state, aether-gateway
- **Key patterns**: CLI app, TUI setup wizard, tunnel protocol, egress filtering
...
| Tool | Purpose | Example |
|------|---------|---------|
| `get_repo_structure` | Full file listing | `get_repo_structure({repo_name: "aether-proxy"})` |
| `get_file_structure` | All nodes in a file | `get_file_structure({repo_name: "aether-proxy", file_path: "src/..."})` |
| `get_ast_node` | Code + deps + refs | `get_ast_node({repo_name: "aether-proxy", node_ids: [...]})` |
...
All files are in `.trellis/spec/aether-proxy/backend/`:
...
- ONLY modify files under `.trellis/spec/aether-proxy/backend/`

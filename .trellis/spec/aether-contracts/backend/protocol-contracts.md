...
- `TRANSPORT_BACKEND_BROWSER_WREQ` at `crates/aether-contracts/src/plan.rs:66`.
- `TRANSPORT_HTTP_MODE_AUTO` and `TRANSPORT_HTTP_MODE_HTTP1_ONLY` at
  `crates/aether-contracts/src/plan.rs:68`.
- `TRANSPORT_POOL_SCOPE_KEY` at `crates/aether-contracts/src/plan.rs:70`.

Use the constants in consumers. Do not duplicate literal strings in gateway,
proxy, or testkit code.

Browser transport backends are an execution capability, not provider protocol
names. `browser_wreq` is the in-process Rust browser impersonation backend.
Execution consumers must branch on the transport backend constant instead of
inspecting provider names.

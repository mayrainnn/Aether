...
When a provider needs a wider request envelope than the generic OpenAI image
surface, keep the default path conservative and add explicit normalization
options. For example, generic OpenAI image normalization keeps `n=1`, while Grok
generation candidates can call the optioned normalizer with a provider-specific
maximum. This avoids widening validation for Gemini, ChatGPT-Web, and future
image providers by accident.

...
- Keep provider capability rules separate from model-test execution. For
  example, `openai:image` generation count is a validation capability used by
  model tests; the actual multi-image generation behavior must remain in the
  provider image runtime.
- `model_test_capabilities["openai:image"]` describes generation and edit
  testing only. Do not expose a `supports_variation` capability: public
  `/v1/images/variations` is intentionally not part of Aether's image gateway
  surface.

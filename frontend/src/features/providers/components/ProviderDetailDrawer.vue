...
// Codex / Gemini CLI / Antigravity / Kiro / ChatGPT Web：打开抽屉后自动后台刷新（配额缓存缺失/过期，或 Token 即将过期时触发）
...
// 检查 OAuth Token 是否即将过期（Codex / Gemini CLI / Antigravity / Kiro / ChatGPT Web）
...
async function autoRefreshQuotaInBackground(options: { ignoreCooldown?: boolean } = {}) {
  ...
  const providerType = provider.value?.provider_type
  if (providerType !== 'codex' && providerType !== 'gemini_cli' && providerType !== 'antigravity' && providerType !== 'kiro' && providerType !== 'chatgpt_web' && providerType !== 'grok') return
